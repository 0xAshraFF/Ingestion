...
const inputDir = "inputs/legal-demo";
const resultDir = "results/legal-demo";
...
const files = await Promise.all(manifest.documents.map(async (doc) => {
  const text = await readFile(join(inputDir, doc.text), "utf8");
...
const csv = [
  "input_doc,model_used,confidence_metric,accuracy_proxy,quality_band,fallback_used",
  ...rows.map((row) => [
    row.sampleDoc,
    row.modelUsed,
...
await writeFile(join(resultDir, "actual_case_fact_summary.md"), draft.content);
await writeFile(join(resultDir, "evaluation-report.md"), report);
await writeFile(join(resultDir, "model-confidence-results.csv"), csv);
...
console.log(JSON.stringify({
  ...
  generated: `${resultDir}/actual_case_fact_summary.md`,
  results: `${resultDir}/model-confidence-results.csv`
}, null, 2));
